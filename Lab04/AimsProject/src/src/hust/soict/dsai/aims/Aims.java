...
public class Aims {
	...
	public static void playMedia(Scanner aims, Cart cart) {
		...
		if (found == -1) {
			...
			if (cart.getItemsOrdered().get(found) instanceof DigitalVideoDisc) {
				DigitalVideoDisc disc = (DigitalVideoDisc) cart.getItemsOrdered().get(found);
				disc.play();
			}
			else if (cart.getItemsOrdered().get(found) instanceof CompactDisc) {
				CompactDisc disc = (CompactDisc) cart.getItemsOrdered().get(found);
				disc.play();
			}
			else {
				System.out.println("Can not play Book");
	...
	public static int findIndex(String title, Cart cart) {
		for (int i = 0; i < cart.getItemsOrdered().size(); i++) {
			if (title == cart.getItemsOrdered().get(i).getTitle()) {
				return i;
			}
		}
		return -1;
